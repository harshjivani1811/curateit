...
module.exports = () => ({
  getDomainDetails: async (url) => {
    ...
    const webcontact = await axios.get(
    ...
    if (urldata.startsWith("https://www.")) {
      const digitalRankData = await axios.get(
        `https://api.similarweb.com/v1/similar-rank/${urldata.slice(
          12
        )}/rank?api_key=${process.env.SIMILAR_RANK_API_KEY}`
      );
      ...
      const digitalRankData = await axios.get(
        `https://api.similarweb.com/v1/similar-rank/${urldata}/rank?api_key=${process.env.SIMILAR_RANK_API_KEY}`
      );
      digitalRes = digitalRankData;
    }
    console.log("result");
